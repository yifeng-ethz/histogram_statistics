...
-- Revision: 1.1
--      Date: May 16, 2026
--      Change: Add a V3 power-of-two-only implementation option. In that
--              profile the divider becomes a range check plus shift, and a
--              non-power-of-two bin width is reported as overflow if it ever
...
architecture rtl of bin_divider is
    ...
    subtype tick_unsigned_t is unsigned(TICK_WIDTH - 1 downto 0);
...
begin
    ...
    power2_divider_gen : if POWER2_BIN_WIDTH_ONLY generate
        signal valid_d0     : std_logic := '0';
        signal underflow_d0 : std_logic := '0';
        signal overflow_d0  : std_logic := '0';
        signal count_d0     : unsigned(COUNT_WIDTH - 1 downto 0) := (others => '0');
        signal user_d0      : std_logic := '0';
        signal delta_d0     : tick_unsigned_t := (others => '0');
        signal shift_d0     : natural range 0 to TICK_WIDTH - 1 := 0;
        signal valid_d1     : std_logic := '0';
        signal underflow_d1 : std_logic := '0';
        signal overflow_d1  : std_logic := '0';
        signal count_d1     : unsigned(COUNT_WIDTH - 1 downto 0) := (others => '0');
        signal user_d1      : std_logic := '0';
        signal bin_index_d1 : unsigned(BIN_INDEX_WIDTH - 1 downto 0) := (others => '0');
    begin
        o_valid     <= valid_d1;
        o_underflow <= underflow_d1;
        o_overflow  <= overflow_d1;
        o_bin_index <= bin_index_d1;
        o_count     <= count_d1;
        o_user      <= user_d1;

        shift_divider_pipe : process (i_clk)
                ...
                if i_rst = '1' or i_clear = '1' then
                    ...
                    if i_valid = '1' then
                        if i_key < i_left_bound then
                            ...
                            overflow_d0 <= '1';
                        elsif not is_power2(i_bin_width) then
                            overflow_d0 <= '1';
                        else
                            delta_d0 <= unsigned(i_key) - unsigned(i_left_bound);
                            for bit_idx in 0 to TICK_WIDTH - 1 loop
                                if i_bin_width(bit_idx) = '1' then
                                    shift_d0 <= bit_idx;
                                end if;
                            end loop;
                        end if;
                    ...
                    valid_d1     <= valid_d0;
                    underflow_d1 <= underflow_d0;
                    overflow_d1  <= overflow_d0;
                    count_d1     <= count_d0;
                    user_d1      <= user_d0;
                    bin_index_v  := (others => '0');

                    if (valid_d0 = '1') and (underflow_d0 = '0') and (overflow_d0 = '0') then
                        for dst_idx in 0 to BIN_INDEX_WIDTH - 1 loop
                            src_idx_v := dst_idx + shift_d0;
                            if src_idx_v < TICK_WIDTH then
                                bin_index_v(dst_idx) := delta_d0(src_idx_v);
                            end if;
                        end loop;
                    end if;

                    bin_index_d1 <= bin_index_v;
                end if;
            end if;
        end process shift_divider_pipe;
    end generate power2_divider_gen;

    restoring_divider_gen : if not POWER2_BIN_WIDTH_ONLY generate
        ...
        signal user_pipe       : std_logic_vector(0 to QUOTIENT_STAGE_COUNT_CONST) := (others => '0');
    ...
    begin
        ...
        o_user      <= user_pipe(QUOTIENT_STAGE_COUNT_CONST);
            ...
            if rising_edge(i_clk) then
                if i_rst = '1' or i_clear = '1' then
                    ...
                    for stage_idx in 0 to QUOTIENT_STAGE_COUNT_CONST - 1 loop
                        ...
                        user_pipe(stage_idx + 1)      <= user_pipe(stage_idx);
    ...
    end generate restoring_divider_gen;
